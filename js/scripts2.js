...
function vd_envia_usuario(){
	...
	// verifica los campos nombre, apellido y telefono
...
function vd_envia_registro(){
	...
	// verifica los campos encargado, responsable, procedencia, sexo, raza
...
function regresar(){
			...
			window.location="./sirce_menu.php";
...
function dt_novo_alumno(){
    window.location="./sirce_regcalific_2.php";
}
...
function dt_bscr_regcalific(){
    mipopup = window.open("./buscar_calificaciones.php","formu","width=200px,heigth=5px,menubar=no,toolbar=no,scrollbars=yes,left=50,top=10,location=no,resizable=no");
    mipopup.focus()
}
...
function dt_grbr_docente(){
    ...
    if ((xcod!="")&&(xfre!="")&&(xape!="")&&(xnom!="")&&(xnac!="")&&(xced!="")&&(xtlc!="")&&(xtlp!="")&&(xdir!="")&&(xema!="")
        ...
        f.action="sircadmon_guardar.php";
        f.submit();
...
//    var opcion=document.getElementByName("correo");
 //   if(opcion.length>1){
...
//            if(opcion[i].checked){
       ...
       // }
  //  }
   // valor=document.getElementByName("desafio").value;
   // if(nvalc==1){valor=1;}
   // if(nvald==2){valor=2;}

  //  alert(valor);
    
//    location.href="sirce_olvido.php?opcion=valor";
}
